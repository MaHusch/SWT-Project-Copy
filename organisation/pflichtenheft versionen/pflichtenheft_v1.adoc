...
|===
|Name |Beschreibung 
|Admin   |greift auf die im Store gespeicherten Daten zu und kann diese verändern   
|Kunde(Customer)   |ruft in der Filiale an, authentifiziert sich mit seiner TAN und gibt seine Bestellung beim Verkäufer auf 
|Baker(Bäker)   |bestückt die Öfen mit Pizzen und achtet auf ihre Timer  
|Seller(Verkäufer)   |nimmt Bestellungen sowie Essgarnituren entgegen, registriert neue Nutzer und überprüft am Anfang der Bestellung die TAN des Kunden   
|Deliverer(Lieferbote)   |fährt die Bestellungen aus und nimmt ggf. Essgarnitur wieder zurück         
|===
