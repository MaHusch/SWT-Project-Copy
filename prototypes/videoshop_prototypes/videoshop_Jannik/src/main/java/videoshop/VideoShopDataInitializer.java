...
public class VideoShopDataInitializer implements DataInitializer {
	...
	private void initializeCatalog(VideoCatalog videoCatalog, Inventory<InventoryItem> inventory) {
		...
		/*Iterable<Disc> test = videoCatalog.findByName("Oldboy");
		Iterator<Disc> testiterator = test.iterator();
		
		List<Disc> list = new ArrayList<Disc>();
		
		list.add(0,testiterator.next());
		
		videoCatalog.save(new Bundle("Bill & Ted's Excellent Adventure", "bt", Money.of(29.99, EURO),
				"Adventure/Comedy/Family", list));
		
		
		System.out.println(testiterator.next().getGenre());
		*/
		// (｡◕‿◕｡)
